...
# parser arguments
parser.add_argument('-in1', type = argparse.FileType('r'), required = True, help = 'Input protein sequence file')
parser.add_argument('-db1', type = str, required = True, help = 'Database type name to give new column to label header id\'s')
...
parser.add_argument('-in2', type = argparse.FileType('r'), required = False, help = '2nd input sequence to merge the database metadata')
parser.add_argument('-db2', type = str, required = False, help = 'Second database type name to give new column to label header id\'s')
parser.add_argument('-out1', type = str, required = True, help = 'Output file name')
parser.add_argument('-out2', type = str, required = False, help = 'Second output file name')
parser.add_argument('-mergedout', type = str, required = False, help = 'Merged output file name')
...
if args.merged_metadata == True:
	...
	database_metainfo.to_csv(f'{args.out1}_metadata.csv',index = False)
	...
	db = [args.db2] * len(genome_id)

	#create a dataframe that contains all information
	database_metainfo = pd.DataFrame({'sequence_header':clean_header, 'genome_id':genome_id, 'gene_number':gene_number, 'database':db})

	# save dataframe as csv file
	database_metainfo.to_csv(f'{args.out2}_metadata.csv',index = False)
	...
	db_1 = pd.read_csv(f'{args.out}_metadata.csv')
	db_2 = pd.read_csv(f'{args.out2}_metadata.csv')
